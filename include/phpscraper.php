...
class PHPScraper
{
    ...
    public function buildURL(        
        ...
        $this->_site = $site;
        if (PHPScraper::isValidDomainName($this->_site)) {
            $this->validSite = true;
        } else {
    ...
    {      
        $postionsArray = array();  
        if (!$this->validSite) {
            $postionsArray['Domain Name NOT VALID'][''] = "NOT TOP";
            $this->prozent = 0;
        ...
        foreach ($this->_keywordsList as $_key => $_value) {
            ...
            for ($i=1; $i<=$this->_pages; $i++) {
                ...
                /** 
                 * Some information about the proces 
                 */
                $actualtime = time();
    ...
    public function isValidDomainName($domainName)
    {
        ...
        if ($domainNameparts['host']!='') {
            ...
            if (checkdnsrr($domainNameparts['host'], 'A') 
                && in_array($domainNameparts['scheme'], array('http', 'https')) 
                && ip2long($domainNameparts['host']) === false
            ) { 
